...
export const DragAndPlusButton = ({ ed: editor, dragHandleWidth }: DragButtonOptions) => {
  ...
  function isCurrentNodeATextNode() {
    ...
    addEventListener('mousemove', (event) => {
      ...
      dragStyleLeft.current = `${rect.left - rect.width}px`;
      dragStyleTop.current = `${rect.top}px`;

      addStyleLeft.current = `${rect.left - rect.width - 24}px`;
      addStyleTop.current = `${rect.top}px`;
      showDragHandle();
    });
